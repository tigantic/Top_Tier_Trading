...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('notional', sa.Numeric(precision=18, scale=8), nullable=False, server_default='0'),
    )
    op.create_table(
        'positions',
        sa.Column('product_id', sa.String(), primary_key=True),
        sa.Column('quantity', sa.Numeric(precision=18, scale=8), nullable=False, server_default='0'),
        sa.Column('average_price', sa.Numeric(precision=18, scale=8), nullable=False, server_default='0'),
    )
    op.create_table(
        'daily_pnl',
